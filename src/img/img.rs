...
async fn random_image_in_group(event:&GroupMessageEvent) -> anyhow::Result<bool>{
    ...
    let img_src = reqwest::get(real_url)
        .await?
        .bytes()
        .await?
        .to_vec();
    let img = event.upload_image_to_source(img_src).await?;
    event.send_message_to_source(reply_chain(event).await).await?;
    event.send_message_to_source(MessageChain::default().append(img)).await?;
    Ok(true)
}
            ...
            match ok_or_not_2 {
                Ok(res)=>{
                    ...
                    match ok_or_not_3 {
                        ...
                        Err(_e)=>{
                            event.send_message_to_source(reply_chain(event).await.append("获取错误，请重试".parse_text())).await?;
                        }
                    }
...
pub fn module() -> Module {
    ...
    module!(
        "img",
        "img",
        random_image_in_group,
        search_image_in_group
    )
}